...
void tokenize() {
    while (src_pos < src_len) {
        ...
        if (c == '|' && src_pos + 2 < src_len && src[src_pos+1] == '|' && src[src_pos+2] == '|') {
            src_pos += 3;
            while (src_pos < src_len && !(src[src_pos] == '|' && src[src_pos+1] == '|' && src[src_pos+2] == '|')) src_pos++;
            src_pos += 3; continue;
        }
        ...
        else if (c == '*') { if (src_pos + 1 < src_len && src[src_pos + 1] == '*') { t.type = TOKEN_POW; strcpy(t.value, "**"); src_pos++; } else if (src_pos + 1 < src_len && src[src_pos + 1] == '=') { t.type = TOKEN_STAR_ASSIGN; strcpy(t.value, "*="); src_pos++; } else t.type = TOKEN_STAR; }
        else if (c == '/') { if (src_pos + 1 < src_len && src[src_pos + 1] == '=') { t.type = TOKEN_SLASH_ASSIGN; strcpy(t.value, "/="); src_pos++; } else t.type = TOKEN_SLASH; }
    ...
    while (currentTokenIndex < tokenCount) {
        ...
        if (t.type == TOKEN_POW) {
            ...
            if (currentTokenIndex + 1 < tokenCount) {
                ...
                if (nt == TOKEN_ASSIGN || nt == TOKEN_MOD_ASSIGN || nt == TOKEN_PLUS_ASSIGN ||
                    nt == TOKEN_MINUS_ASSIGN || nt == TOKEN_STAR_ASSIGN || nt == TOKEN_SLASH_ASSIGN) {
                    break;
                }
            }
            fprintf(out, "%s ", next_token().value);
...
void compile_condition(FILE* out) {
    int depth = 0; next_token();
    while (currentTokenIndex < tokenCount) {
        Token t = peek_token();
        if (t.type == TOKEN_LPAREN) depth++;
        else if (t.type == TOKEN_RPAREN) { if (depth == 0) { next_token(); break; } depth--; }
        fprintf(out, "%s ", next_token().value);
    }
}
...
void compile_statement(FILE* out) {
    ...
    if (tok.type == TOKEN_FN) {
        int saved_count = declared_count; declared_count = 0;
        Token name = next_token();
        fprintf(out_funcs, "double %s(", name.value); next_token();
        int first = 1;
        while (peek_token().type != TOKEN_RPAREN) {
            if (!first) { fprintf(out_funcs, ", "); next_token(); }
            Token param = next_token(); fprintf(out_funcs, "double %s", param.value);
            declare_var(param.value); first = 0;
        }
        next_token(); fprintf(out_funcs, ") {\n"); next_token();
        while (currentTokenIndex < tokenCount) {
            Token t = peek_token();
        ...
        fprintf(out, "return "); compile_expression(out); fprintf(out, ";\n");
        ...
        if (next.type == TOKEN_STRING) {
            ...
            if (is_pow) {
                ...
                fprintf(out, "%s ", next_token().value); next_token();
                fprintf(out, ", %s", next_token().value); fprintf(out, "));\n");
            } else {
                fprintf(out, "printf(\"%%g\\n\", (double)("); compile_expression(out); fprintf(out, "));\n");
            }
        }
    } else if (tok.type == TOKEN_CONST) {
        Token name_tok = next_token(); next_token();
        fprintf(out, "const double %s = ", name_tok.value); compile_expression(out); fprintf(out, ";\n");
        declare_var(name_tok.value);
    } else if (tok.type == TOKEN_LET) {
        Token name_tok = next_token(); next_token(); Token val_peek = peek_token();
        if (!is_declared(name_tok.value)) {
            if (val_peek.type == TOKEN_LBRACKET) fprintf(out, "int %s[] = ", name_tok.value);
            else if (val_peek.type == TOKEN_FLOAT) fprintf(out, "double %s = ", name_tok.value);
            else fprintf(out, "int %s = ", name_tok.value);
            declare_var(name_tok.value);
        } else fprintf(out, "%s = ", name_tok.value);

        if (val_peek.type == TOKEN_INPUT) {
            next_token(); fprintf(out, "0; printf(\"> \"); scanf(\"%%d\", &%s);\n", name_tok.value);
        } else if (val_peek.type == TOKEN_LBRACKET) {
            next_token(); fprintf(out, "{");
            while (peek_token().type != TOKEN_RBRACKET) fprintf(out, "%s", next_token().value);
            next_token(); fprintf(out, "};\n");
        } else {
        ...
        if (op.type == TOKEN_MOD_ASSIGN) {
            fprintf(out, "%s = (int)%s %% (int)(", tok.value, tok.value); compile_expression(out); fprintf(out, ");\n");
        } else {
            fprintf(out, "%s %s ", tok.value, op.value); compile_expression(out); fprintf(out, ";\n");
        }
    } else if (tok.type == TOKEN_IF) {
        fprintf(out, "if ("); compile_condition(out); fprintf(out, ") {\n"); next_token(); compile_block(out);
        while (peek_token().type == TOKEN_ELIF) {
            next_token(); fprintf(out, "else if ("); compile_condition(out); fprintf(out, ") {\n"); next_token(); compile_block(out);
        }
        if (peek_token().type == TOKEN_ELSE) { next_token(); fprintf(out, "else {\n"); next_token(); compile_block(out); }
    } else if (tok.type == TOKEN_WHILE) {
        fprintf(out, "while ("); compile_condition(out); fprintf(out, ") {\n"); next_token(); compile_block(out);
    } else if (tok.type == TOKEN_FOR) {
        fprintf(out, "for ("); next_token(); int depth = 0;
        while (currentTokenIndex < tokenCount) {
            Token t = peek_token();
            if (t.type == TOKEN_LPAREN) depth++;
            else if (t.type == TOKEN_RPAREN) { if (depth == 0) { next_token(); break; } depth--; }
            else if (t.type == TOKEN_COMMA) { next_token(); fprintf(out, "; "); continue; }
            else if (t.type == TOKEN_LET) {
                next_token(); Token name_tok = next_token(); next_token();
                fprintf(out, "int %s = ", name_tok.value); declare_var(name_tok.value); continue;
            }
            fprintf(out, "%s ", next_token().value);
        }
        fprintf(out, ") {\n"); next_token(); compile_block(out);
    } else if (tok.type == TOKEN_BREAK) { fprintf(out, "break;\n"); }
    else if (tok.type == TOKEN_CONTINUE) { fprintf(out, "continue;\n"); }
    else if (tok.type == TOKEN_SWITCH) {
        fprintf(out, "switch ("); compile_condition(out); fprintf(out, ") {\n"); next_token();
        while (currentTokenIndex < tokenCount) {
            Token t = peek_token();
            if (t.type == TOKEN_RBRACE) { next_token(); fprintf(out, "}\n"); break; }
            if (t.type == TOKEN_CASE) {
                next_token(); fprintf(out, "case "); compile_expression(out); fprintf(out, ": {\n"); next_token(); compile_block(out); fprintf(out, "break;\n");
            } else if (t.type == TOKEN_DEFAULT) { next_token(); fprintf(out, "default: {\n"); next_token(); compile_block(out); }
            else next_token();
        }
    }
}
...
int main(int argc, char** argv) {
    if (argc == 2 && (strcmp(argv[1], "--version") == 0 || strcmp(argv[1], "-v") == 0)) { printf("Gage Programming Language v3.1.1\n"); return 0; }
    if (argc < 2) { printf("Usage: gage <filename.gg>\n"); return 1; }
    FILE* file = fopen(argv[1], "r"); if (!file) { printf("Error: Could not open file %s\n", argv[1]); return 1; }
    ...
    tokenize();
    char* temp_dir = getenv("TMPDIR"); if (!temp_dir) temp_dir = "/tmp";
    char path_funcs[512], path_main[512], path_temp[512], path_exec[512];
    sprintf(path_funcs, "%s/.gage_funcs.c", temp_dir); sprintf(path_main, "%s/.gage_main.c", temp_dir);
    sprintf(path_temp, "%s/.gage_temp.c", temp_dir); sprintf(path_exec, "%s/.gage_executable", temp_dir);

    out_funcs = fopen(path_funcs, "w"); out_main = fopen(path_main, "w");
    ...
    FILE* out_c = fopen(path_temp, "w"); if (!out_c) { printf("Error: Could not write build template.\n"); return 1; }
    fprintf(out_c, "#include <stdio.h>\n#include <stdlib.h>\n#include <math.h>\n");
    FILE* f_in = fopen(path_funcs, "r"); int c; while ((c = fgetc(f_in)) != EOF) fputc(c, out_c); fclose(f_in);
    fprintf(out_c, "int main() {\n");
    FILE* m_in = fopen(path_main, "r"); while ((c = fgetc(m_in)) != EOF) fputc(c, out_c); fclose(m_in);
    fprintf(out_c, "return 0;\n}\n"); fclose(out_c); free(src);

    printf("\033[1;32m   Compiling\033[0m target logic via clang...\n");
    char compile_cmd[1024]; sprintf(compile_cmd, "clang %s -o %s -lm 2>/dev/null", path_temp, path_exec);
    if (system(compile_cmd) == 0) {
        char run_cmd[512]; sprintf(run_cmd, "%s", path_exec); system(run_cmd);
    } else {
        printf("\033[1;31m      Error\033[0m: Target transpile parsing failed.\n");
    }
    char clean_cmd[1024]; sprintf(clean_cmd, "rm -f %s %s %s %s", path_funcs, path_main, path_temp, path_exec);
    system(clean_cmd); return 0;
}
